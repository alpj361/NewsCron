...
# Script para probar la API de Gemini (versión corregida)
...
if ! command -v jq &> /dev/null; then
    ...
    echo "Instala jq: brew install jq (en macOS) o apt-get install jq (en Ubuntu)"
    exit 1
fi
...
# Crear el prompt de prueba (sin comillas escapadas problemáticas)
...
echo "📝 Prompt preparado (${#PROMPT} caracteres)"

# Crear el JSON request usando jq para evitar problemas de escapado
REQUEST_JSON=$(jq -n \
  --arg text "$PROMPT" \
  '{
    contents: [
...
if [ "$HTTP_STATUS" -eq 200 ]; then
    ...
    # Mostrar algunos detalles del request para debug
    echo ""
    echo "🔍 Debug info:"
    echo "Request size: $(echo "$REQUEST_JSON" | wc -c) bytes"
    echo "Prompt size: ${#PROMPT} chars"
fi
